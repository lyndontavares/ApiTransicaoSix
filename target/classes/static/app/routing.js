!function($, jQuery, window, document) {
	...
	app.module.config(function ($stateProvider, $urlRouterProvider) {
		$stateProvider
			...
			.state('fatura', getState('fatura'))

			.state('grafico',getState('grafico'))
			.state('sincro',getState('sincro'))
			.state('campo',getState('campo'))

			.state('info',getState('info'))
			
			.state('wizard', getState('wizard'))
			.state('wizard2', getState('wizard2'))
			.state('wizard3', getState('wizard3'))
